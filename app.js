...
function setupStaticOptions() {
  ...
  if (monthSelect && !monthSelect.options.length) {
    ...
    for (let month = 1; month <= 12; month += 1) {
      ...
      option.textContent = new Date(2026, month - 1, 1).toLocaleString('en-US', { month: 'long' });
...
function isValidExecUrl(value) {
  ...
  return /^https:\/\/script\.google\.com\/macros\/s\/[\w-]+\/exec$/i.test(normalized);
}
...
function updateApiWarning() {
  ...
  warning.textContent = 'API URL is not configured. Open API Settings and save your deployed Google Apps Script /exec URL.';
}
...
function updateActionAvailability() {
  ...
  ['addTaskBtn', 'addTaskBtnTop', 'refreshBtn', 'exportBtn'].forEach(id => {
...
function clearSettings() {
  ...
  document.getElementById('apiBaseUrlInput').value = '';
  refreshConnectionUi();
  setSyncStatus('API URL cleared');
  updateLastUpdated(null);
  showToast('Saved API URL removed', 'info');
}

async function testConnection() {
  const apiUrl = getApiBaseUrl();
  if (!isValidExecUrl(apiUrl)) {
    showToast('Please enter a valid Apps Script /exec URL first', 'error');
    return;
  }
...
async function loadTasks() {
  ...
  showLoading(true);
  setSyncStatus('Loading tasks from Google Sheets...');
  ...
  try {
    ...
    applyFilters();
    setSyncStatus(`Synced ${state.tasks.length} task${state.tasks.length === 1 ? '' : 's'}`);
    updateLastUpdated(new Date());
  } catch (error) {
    console.error(error);
    state.tasks = [];
    ...
    showToast(error.message || 'Unable to load tasks. Check API access and deployment settings.', 'error');
  } finally {
    showLoading(false);
  }
}
...
function applyFilters() {
  ...
  state.filteredTasks = state.tasks.filter(task => {
    const text = [task.taskName, task.partner, task.assignedTo, task.notes, task.category, task.risk]
      .join(' ')
      .toLowerCase();
    ...
    if (month) {
      ...
      if (startMonth && endMonth) {
        matchesMonth = targetMonth >= startMonth && targetMonth <= endMonth;
      } else if (startMonth) {
        matchesMonth = targetMonth === startMonth;
      } else if (endMonth) {
        matchesMonth = targetMonth === endMonth;
      } else {
        matchesMonth = false;
      }
    }
...
function renderRoadmap() {
  ...
  const tasks = [...state.filteredTasks].sort(sortByRoadmap);

  if (!board || !summary) return;

  if (!tasks.length) {
    summary.innerHTML = `
      <div class="roadmap-chip"><strong>0</strong> visible tasks</div>
      <div class="roadmap-chip">Apply filters or add a new task</div>
    `;
    board.innerHTML = `<div class="empty-state roadmap-empty">${escapeHtml(isApiConfigured() ? 'No matching tasks found' : 'Configure API URL to load tasks')}</div>`;
    return;
  }

  const activeMonths = new Set();
  tasks.forEach(task => {
    const startMonth = clampMonth(getMonthFromDate(task.startDate) || getMonthFromDate(task.endDate) || 1);
    const endMonth = clampMonth(getMonthFromDate(task.endDate) || getMonthFromDate(task.startDate) || startMonth);
    for (let month = startMonth; month <= endMonth; month += 1) activeMonths.add(month);
  });

  summary.innerHTML = `
    <div class="roadmap-chip"><strong>${tasks.length}</strong> visible tasks</div>
    <div class="roadmap-chip"><strong>${activeMonths.size}</strong> active months</div>
    <div class="roadmap-chip"><span class="legend-dot status-in-progress"></span>In Progress</div>
    <div class="roadmap-chip"><span class="legend-dot status-completed"></span>Completed</div>
    <div class="roadmap-chip"><span class="legend-dot status-on-hold"></span>On Hold</div>
    <div class="roadmap-chip"><span class="legend-dot status-not-started"></span>Not Started</div>
  `;

  const headerMonths = MONTH_NAMES.map((month, index) => `
    <div class="roadmap-month ${isCurrentMonth(index + 1) ? 'current' : ''}">${month}</div>
  `).join('');

  const rows = tasks.map(task => renderRoadmapRow(task)).join('');

  board.innerHTML = `
    <div class="roadmap-grid roadmap-header-row">
      <div class="roadmap-task-head">Task / owner</div>
      <div class="roadmap-track-head">${headerMonths}</div>
    </div>
    ${rows}
  `;
}

function renderRoadmapRow(task) {
  const startMonth = clampMonth(getMonthFromDate(task.startDate) || getMonthFromDate(task.endDate) || 1);
  const endMonth = clampMonth(getMonthFromDate(task.endDate) || getMonthFromDate(task.startDate) || startMonth);
  const span = Math.max(1, (endMonth - startMonth) + 1);
  const statusClass = `status-${String(task.status || 'Not Started').toLowerCase().replace(/\s+/g, '-')}`;
  const overdueClass = checkOverdue(task) ? 'overdue' : '';
  const monthCells = Array.from({ length: 12 }, (_, index) => {
    const month = index + 1;
    return `<div class="roadmap-cell ${isCurrentMonth(month) ? 'current' : ''}"></div>`;
  }).join('');

  return `
    <div class="roadmap-grid roadmap-row ${overdueClass}">
      <div class="roadmap-task-card">
        <div class="roadmap-task-top">
          <div>
            <div class="roadmap-task-title">${escapeHtml(task.taskName || 'Untitled task')}</div>
            <div class="roadmap-task-meta">${escapeHtml(task.category || 'No category')} · ${escapeHtml(task.assignedTo || 'Unassigned')}</div>
          </div>
...
function renderAllTasksTable() {
  ...
  if (!tasks.length) {
    tbody.innerHTML = emptyRow(10, isApiConfigured() ? 'No tasks found' : 'Configure API URL to load tasks');
    return;
  }
...
function exportTasks() {
  const headers = ['Task Name', 'Category', 'Partner', 'Assigned To', 'Start Date', 'End Date', 'Status', 'Risk', 'Reference Link', 'Notes'];
...
function renderRiskBadge(risk) {
  ...
  let cls = 'risk-custom';
  if (normalized === 'low') cls = 'risk-low';
  if (normalized === 'medium') cls = 'risk-medium';
  if (normalized === 'high') cls = 'risk-high';
...
function sortByRoadmap(a, b) {
  const aTime = sortDateValue(a.startDate || a.endDate);
  const bTime = sortDateValue(b.startDate || b.endDate);
